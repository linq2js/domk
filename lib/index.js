...
function updateNode(node, bindingKey, context, result) {
  ...
  if (result.on) {
    Object.entries(result.on).forEach(([name, value]) =>
      updateListener(node, node.__model, name, value)
    );
  }
...
function updateChildNodes(
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (!childNode) {
      ...
      bindingData.keyList.push(key);
    }
    keyList[i] = key;
    nodeMap[key] = childNode;
    delete bindingData.nodeMap[key];
  }
  const removedKeys = Object.keys(bindingData.nodeMap);
  for (let i = 0; i < removedKeys.length; i++) {
    const key = removedKeys[i];
    const childNode = bindingData.nodeMap[key];
    animations.out.length && startAnimation(key, childNode, animations.out);
    parentNode.removeChild(childNode);
  }
  // collect updated node list
  const remainingKeyList = bindingData.keyList.filter((key) => key in nodeMap);

  for (let i = 0; i < keyList.length; i++) {
    const childModel = isArray ? childModels[i] : i;
    const childNode = nodeMap[keyList[i]];
    const key = keyList[i];
    if (key !== remainingKeyList[i]) {
      parentNode.insertBefore(childNode, nodeMap[remainingKeyList[i + 1]]);
      updateChildNode(childNode, childModel, context, update);
    } else {
      updateChildNode(childNode, childModel, context, update);
    }
    if (childNode.__isNew) {
      delete childNode.__isNew;
      animations.in.length && startAnimation(key, childNode, animations.in);
    }
  }
...
function updateListener(node, prev, name, value) {
  const key = "e:" + name;
