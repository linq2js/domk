...
function applyAnimation(node, animations) {
  for (let i = 0; i < animations.length; i++) {
    const { delay, duration, ease, from, to, custom, update } = animations[i](
      node
    );
    if (custom) continue;
    tween(from, to, {
...
function updateChildNodes(
  ...
  for (let i = 0; i < removedNodes.length; i++) {
    animations.out.length && applyAnimation(removedNodes[i], animations.out);
  ...
  for (let i = 0; i < keyList.length; i++) {
    ...
    if (keyList[i] !== remainingKeyList[i]) {
      updateChildNode(childNode, childModel, context, update);
      animations.out.length && applyAnimation(childNode, animations.out);
      node.insertBefore(childNode, nodeMap[remainingKeyList[i + 1]]);
      animations.in.length && applyAnimation(childNode, animations.in);
    } else {
      updateChildNode(childNode, childModel, context, update);
